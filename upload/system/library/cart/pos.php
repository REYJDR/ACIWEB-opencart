...
class Pos {
	...
	public function getProducts() {
		if (!$this->data) {
			foreach ($this->session->data['cart'] as $key => $quantity) {
				...
				// 24 05 2018 //
					if(isset($this->session->data['pricecart'][$key]))
					{
						$price = $this->session->data['pricecart'][$key];
						$option_price=0;
					}
				// 24 05 2018 //
				...
				else if(!empty($product['cproduct_id']))
				{
					$product_id=$product['cproduct_id'];
					$product_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "pos_product  WHERE product_id = '" . (int)$product_id . "' AND status = '1'");
				...
				// 24 05 2018 //
					if(isset($product_query->row['price']))
					{
						$price = $product_query->row['price'];
					}
				// 24 05 2018 //
					...
					$this->data[$key] = array(
						'key'             => $key,
						'cproduct_id'      => $product_query->row['product_id'],
						'product_id'      => '',
						'name'            => $product_query->row['name'],
						'model'           => $product_query->row['model'],
						'shipping'        => $product_query->row['shipping'],
	...
	public function add($product_id, $qty = 1, $option = array(), $recurring_id = 0,$cproduct_id=0) {
		$this->data = array();

		$product['product_id'] = (int)$product_id;
		if(!empty($cproduct_id))
		{
			$product['cproduct_id']=$cproduct_id;
		}
		if ($option) {
	...
	public function update($key, $qty,$price) {
		$this->data = array();

		if ((int)$qty && ((int)$qty > 0) && isset($this->session->data['cart'][$key])) {
			$this->session->data['cart'][$key] = (int)$qty;
			if(!empty($price)){
			$this->session->data['pricecart'][$key] = $price;
			}
		} else {
			$this->remove($key);
		}
